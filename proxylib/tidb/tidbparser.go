...
func (rule *r2d2Rule) Matches(data interface{}) bool {
	...
	if rule.databaseRegexCompiled != nil &&
		...
		log.Infof("R2d2Rule: file mismatch %s, %s", rule.databaseRegexCompiled.String(), reqData.database)
		return false
	}
...
func ruleParser(rule *cilium.PortNetworkPolicyRule) []proxylib.L7NetworkPolicyRule {
	...
	for _, l7Rule := range allowRules {
		...
		if (rr.databaseRegexCompiled != nil) && !(rr.cmdExact == "" || rr.cmdExact == "select" || rr.cmdExact == "insert") {
...
func (f *factory) Create(connection *proxylib.Connection) interface{} {
	log.Infof("R2d2ParserFactory: Create: %v", connection)
