...
func (rule *TiDBRule) Matches(data interface{}) bool {
	...
	if rule.queryActionExact != "" && rule.queryActionExact != parts[2] {
		log.Debugf("TiDBRule: query_action mismatch %v, %s", rule.queryActionExact, parts[1])
		return false
	}
	if len(parts[3]) > 0 &&
		rule.tableRegexCompiled != nil &&
		!rule.tableRegexCompiled.MatchString(parts[3]) {
		log.Debugf("TiDBRule: table_regex mismatch '%v', '%s'", rule.tableRegexCompiled, parts[3])
		return false
	}
...
// TiDBRuleParser parses protobuf L7 rules to enforcement objects
// May panic
func TiDBRuleParser(rule *cilium.PortNetworkPolicyRule) []L7NetworkPolicyRule {
	...
	for _, l7Rule := range allowRules {
		...
		if len(cr.queryActionExact) > 0 {
			...
			if res == invalidAction {
				ParseError(fmt.Sprintf("Unable to parse L7 tidb rule with invalid query_action: '%s'", cr.queryActionExact), rule)
		...
		log.Debugf("Parsed TiDBRule pair: %v", cr)
		rules = append(rules, &cr)
	}
	return rules
}

type TiDBParserFactory struct{}

var tidbParserFactory *TiDBParserFactory

func init() {
	log.Debug("init(): Registering tidbParserFactory")
	RegisterParserFactory("tidb", tidbParserFactory)
	RegisterL7RuleParser("tidb", TiDBRuleParser)
}
...
func (pf *TiDBParserFactory) Create(connection *Connection) interface{} {
	log.Debugf("TiDBParserFactory: Create: %v", connection)

	p := TiDBParser{connection: connection}
	p.preparedQueryPathByStreamID = make(map[uint16]string)
	p.preparedQueryPathByPreparedID = make(map[string]string)
	return &p
}

func (p *TiDBParser) OnData(reply, endStream bool, dataArray [][]byte) (OpType, int) {
	...
	if reply {
		...
		tidbParseReply(p, data[0:(cassHdrLen+requestLen)])

		log.Debugf("reply, passing %d bytes", (cassHdrLen + requestLen))
		return PASS, (cassHdrLen + int(requestLen))
	}

	err, paths := tidbParseRequest(p, data[0:(cassHdrLen+requestLen)])
	if err != 0 {
...
// "unauthorized" error to be sent to tidb client as part of policy
...
// "unprepared" error to be sent to tidb client if proxy
...
func parseQuery(p *TiDBParser, query string) (string, string) {
	var action string
	var table string
	...
	// characters for tidb, we fail parsing.
...
func tidbParseRequest(p *TiDBParser, data []byte) (OpError, []string) {
...
func tidbParseReply(p *TiDBParser, data []byte) {
